...
<!-- comet-task-ref:2b1aefdd-eea5-4b88-830a-1de3ae22a0be -->
...
- **Step 1: 在 `gradle/libs.versions.toml` 的 `[libraries]` 末尾新增条目**
...
- **Step 2: 在 `shared/build.gradle.kts` 的 `kotlin { }` 块内、`android { }` 块之后声明 iOS target 与 framework**
...
<!-- comet-task-ref:eee405d9-9fac-499f-8043-7559e068f280 -->
...
- **Step 1: 新建 17 个骨架文件**（签名照抄 commonMain expect，逐字如下）
...
<!-- comet-task-ref:aa93ea45-8fc6-47a4-a503-a01a36e623b1 -->
...
- **Step 2: 记录结论**（无代码变更则无提交；若修了回归，单独提交修复）

---

### Task 4: PersistentStorage（NSUserDefaults）与 Clipboard（UIPasteboard）真实现【tasks 2.1】
<!-- comet-task-ref:ba34eeb1-912a-4b7a-a07c-23a8dbd2185f -->
...
- **Step 1: 用以下内容替换 `PersistentStorage.ios.kt`**
...
- **Step 2: 用以下内容替换 `Clipboard.ios.kt`**
...
<!-- comet-task-ref:535f9eae-29a1-4210-9260-133149a361ef -->
...
- **Step 1: 用以下内容替换 `util/TimeUtils.ios.kt`**
...
- **Step 4: 用以下内容替换 `ImageConversion.ios.kt`**
...
- **Step 5: 用以下内容替换 `data/viewmodel/NetworkErrors.ios.kt`**
...
- **Step 6: 用以下内容替换 `ui/theme/DynamicColorKey.ios.kt`**
...
<!-- comet-task-ref:80f05ba9-74e6-4c29-9860-774f638ef926 -->
...
- **Step 1: 用以下内容替换 `data/api/HttpClientProvider.ios.kt`**
...
- **Step 2: 在 `update/AppUpdatePlatform.ios.kt` 中将 `createUpdateHttpClient` 替换为**
...
<!-- comet-task-ref:f3e4bfc4-86d4-48af-96af-47449d3f53be -->
...
- **Step 1: 用以下内容替换 `ui/page/device/QrScannerPage.ios.kt`**
...
- **Step 2: 用以下内容替换 `util/Sponsor.ios.kt`**
...
<!-- comet-task-ref:5d4a473e-01fc-46a6-a8b1-22ae843d4a77 -->
...
- **Step 2: 写入核对清单 `expect-actual-checklist.md`**（内容照抄下表）
...
<!-- comet-task-ref:450c0e8c-470d-49c3-b720-72d57f527ca2 -->
...
- **Step 1: 在 `shared/build.gradle.kts` 文件头部（plugins 之前）添加 import，并在 `kotlin { }` 块之前添加生成任务**
...
- **Step 2: 将生成目录注册进 iosMain sourceSet，并挂接 iOS 编译任务依赖**
...
- **Step 4: 特殊字符转义往返验证（Review Focus #1 的钉子）**
...
<!-- comet-task-ref:686d03b1-908f-4e84-a071-0b6cef6537cb -->
...
- **Step 1: 新建 `shared/src/iosMain/kotlin/com/github/ilife798/MainViewController.kt`**（对齐 MainActivity.onCreate 初始化序列，design §2.3）
...
- **Step 2: 新建 `iosApp/Configuration/Config.xcconfig`**
...
- **Step 3: 新建 `iosApp/iosApp/iOSApp.swift`**
...
- **Step 4: 新建 `iosApp/iosApp/ContentView.swift`**
...
- **Step 5: 新建 `iosApp/iosApp/Info.plist`**（基础声明；相机权限归 Change 2，不在此添加）
...
- **Step 7: 新建 `iosApp/iosApp.xcodeproj/project.pbxproj`**（objectVersion 77 / 文件系统同步组布局，与 Xcode 16 官方模板一致）
...
<!-- comet-task-ref:f862ac93-17c4-4d1a-90b2-ba7b7b84e45c -->
...
- **Step 2: 执行冒烟清单并记录**（每项标 通过/失败/受阻 + 结论；允许附截图路径）
...
<!-- comet-task-ref:d40472ed-fb67-4e7b-8417-0d16ea81a7c0 -->
...
- **Step 3: 无签名环境验证（Review Focus #3 的钉子）**
...
<!-- comet-task-ref:6a5674cb-3d96-4df3-80a7-d09f4ef77f5b -->
...
- **Step 1: 在 `README.md` 追加以下小节（Markdown，二级标题位置按现有文档层级并入）**
...
<!-- comet-task-ref:79fd51a7-7e07-4c1e-b309-c67cc99a467f -->
...
- **Step 1: 创建 `.github/workflows/ios.yml`（先只含 validate job）**
...
<!-- comet-task-ref:a2808944-fb3c-4ce1-ac89-eb8f8532bd25 -->
...
- **Step 1: 在 `.github/workflows/ios.yml` 的 `jobs:` 下追加 release job**
...
<!-- comet-task-ref:ab66c304-ea4c-418c-b99d-3d07a166f9f5 -->
...
- **Step 1: Android CI 全量门（与 android.yml validate 相同任务集）**
...
- **Step 3: PR 文件边界核对（与 Comet 工作流文件完全分离）**
...
- **Step 4: 无代码变更则无提交；若核对发现问题，回对应任务修复后重跑 Step 1-3**
